...
class CartPage extends StatefulWidget {
  const CartPage(
      {Key? key, this.drink, this.antal, this.kurv, required this.index})
      : super(key: key);
...
class _CartPageState extends State<CartPage>
  ...
  void saveInfo(double debt, int index) async {
    ...
    Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => LoadingUser(index: widget.index!)));
  }
    ...
    return Scaffold(
      ...
      body: Container(
        child: Stack(
          children: [
            Padding(
              padding: const EdgeInsets.fromLTRB(0, 70.0, 0, 70.0),
              child: ListView.builder(
                  scrollDirection: Axis.vertical,
                  itemCount: widget.kurv?.length,
                  itemBuilder: (context, index) {
                    return Card(
                      color: selectedDrinksIndex.contains(index)
                          ? Colors.green[100]
                          : Colors.white,
                      child: ListTile(
                        onTap: () {
                          ...
                          selectedDrinksIndex.contains(index)
                              ? selectedDrinksIndex.remove(index)
                              : selectedDrinksIndex.add(index);
                          setState(() {
                            selectedIndex = index;
                          });
                        },
            ...
            Container(
              child: Row(
                children: [
                  Expanded(
                    child: Container(
                      color: selectedDrinksIndex.isEmpty
                          ? Colors.green[400]
                          : Colors.green[100],
                      height: bottomHeight,
                      child: TextButton(
                      ...
                      child: TextButton(
                        onPressed: () {
                          if (selectedDrinksIndex.isNotEmpty) {
                            ...
                            for (int count = selectedDrinksIndex.length - 1;
                                count >= 0;
                                count--) {
