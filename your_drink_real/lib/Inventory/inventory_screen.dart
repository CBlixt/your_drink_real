  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return Card(
                    child: ListTile(
                      title:
                          Text(items[index].name + " x ${items[index].number}"),
                      subtitle: Text("${items[index].price} kr."),
                      trailing: IconButton(
                        icon: Icon(Icons.delete),
                        onPressed: () {
                          setState(() {
                            InventoryFileManager.deleteInventoryItem(index);
                          });
                        },
                      ),
                      onTap: () async {
                      ...
                      leading: CircleAvatar(
                        backgroundImage:
                            AssetImage("assets/${items[index].flag}"),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          setState(() {
            InventoryFileManager.addInventoryItem(InventoryItem(
                name: "New item nr. ${items.length + 1} ",
                number: 0,
                price: 0.0,
